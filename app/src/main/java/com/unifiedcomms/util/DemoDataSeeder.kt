...
object DemoDataSeeder {
    ...
    private suspend fun seed(context: Context) {
        ...
        val conversation = Conversation(participantIds = listOf("current_user", a1, a2), participantNames = mapOf("current_user" to "Demo User", a1 to "Alice Example", a2 to "Bob Example"), title = "Demo group")
        val convId = db.conversationDao().insert(conversation)
        db.messageDao().insert(Message(conversationId = convId.toString(), senderId = a1, recipientId = "current_user", content = "Welcome to UnifiedComms demo."))
        db.messageDao().insert(Message(conversationId = convId.toString(), senderId = "current_user", recipientId = a1, content = "Demo account active. Calendar and tasks are visible on their tabs."))
    }
